...
while(True):
    ...
    if(time<0):
        time=0   
    cv2.putText(frame,'Score:'+str(time),(100,height-20), font, 1,(255,255,255),1,cv2.LINE_AA)
    if(time>=16):
        #person is feeling dizzy we will alert :
        cv2.imwrite(os.path.join(path,'caught.jpg'),frame)
        cv2.putText(frame,'ALERT',(250,height-400), font, 2,(0,0,255),1,cv2.LINE_AA)    
        try:
            alarm_sound.play()
    ...
    cv2.imshow('Driver Drowsiness by Sankalp & Viral',frame)
